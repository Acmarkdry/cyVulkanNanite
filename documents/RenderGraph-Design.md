...
## Render Graph的核心职责
在最开始使用vulkan example进行nanite开发的时候，发现最恶心的事情就是要完全手动梳理一番每个pass之间的资源引用情况，因为需要做好他们之间的resource同步。
所以我想要的Render Graph，最核心的就是自动化处理vulkan的同步和资源管理问题，加上布局转换。
## Legit Engine的缺点
...
resource分配一个id。因为声明pass的时候资源还不存在，所以用proxy id作为一个占位符，
execute阶段再从缓存池分配真实资源，从而填充resolve image
handles -> unique handle raii封装
shader memory pool -> uniform数据管理
## 外包装Render的使用
目前我看到过的demo对于多个渲染demo example的使用，基本都是通过：glfw做一个窗口，内部跑一个baseRender派生出来的可替换的单例去做的。
