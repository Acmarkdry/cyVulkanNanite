...
namespace cyRenderGraph
{
	...
	{
		RenderGraphProxyId() : id(static_cast<size_t>(-1))
		{
		}
	...
	struct ImageKey
	{
		...
		{
			auto lUsageFlags = static_cast<VkMemoryMapFlags>(usageFlags);
			auto rUsageFlags = static_cast<VkMemoryMapFlags>(other.usageFlags);
	...
	public:
		...
		ImageData* GetImage(ImageKey imageKey)
		{
			auto& cacheEntry = imageCache[imageKey];
			if (cacheEntry.usedCount + 1 > cacheEntry.images.size())
			{
				vk::ImageCreateInfo imageCreateInfo;
				if (imageKey.size.z == static_cast<glm::u32>(-1))
					imageCreateInfo = Image::CreateInfo2d(glm::uvec2(imageKey.size.x, imageKey.size.y), imageKey.mipsCount, imageKey.arrayLayersCount, imageKey.format, imageKey.usageFlags);
				else
					// TODO 这里没有看懂，哪里有3d的image
					imageCreateInfo = Image::CreateInfoVolume(imageKey.size, imageKey.mipsCount, imageKey.arrayLayersCount, imageKey.format, imageKey.usageFlags);
				auto newImage = std::unique_ptr<Image>(new Image(physicalDevice, logicalDevice, imageCreateInfo));
	...
	public:
		...
		ImageView* GetImageView(ImageViewKey imageViewKey)
		{
			auto& imageView = imageViewCache[imageViewKey];
			if (!imageView)
				imageView = std::unique_ptr<ImageView>(new ImageView(
					logicalDevice,
					imageViewKey.image,
	...
	private:
		std::map<ImageViewKey, std::unique_ptr<ImageView>> imageViewCache;
		vk::PhysicalDevice physicalDevice;
		vk::Device logicalDevice;
	};
	...
	// ── Proxy structs (merged from ResourceProxy.h) ─────────────────────────
	struct ImageProxy;
	...
	struct BufferProxy
	{
		enum struct Types { Transient, External };
		BufferCache::BufferKey  bufferKey;
		Buffer                 *externalBuffer  = nullptr;
		Buffer                 *resolvedBuffer  = nullptr;
		Types                   type;
	};
	...
	struct ImageProxy
	{
		enum struct Types { Transient, External };
		ImageKey    imageKey;
		ImageData  *externalImage   = nullptr;
		ImageData  *resolvedImage   = nullptr;
		Types       type;
	};
	...
	struct ImageViewProxy
	{
		...
		{
			if (type == Types::Transient
				&& subresourceRange.Contains(other.subresourceRange)
				&& type == other.type
		...
		ImageSubresourceRange  subresourceRange;
		ImageView             *externalView       = nullptr;
		ImageUsageTypes        externalUsageType;
		ImageView             *resolvedImageView  = nullptr;
		std::string            debugName;
		Types                  type;
	};

	// HandleInfo structs – forward-declared RenderGraph is enough here because
	// the method bodies that call RenderGraph member functions are inlined below
	// after the full RenderGraph class definition.
	struct ImageHandleInfo
	{
		ImageHandleInfo() {}
		ImageHandleInfo(RenderGraph* renderGraph, ImageProxyId imageProxyId)
			: renderGraph(renderGraph), imageProxyId(imageProxyId) {}

		void        Reset();
		ImageProxyId Id() const { return imageProxyId; }
	...
	struct ImageViewHandleInfo
	{
		...
		ImageViewHandleInfo(RenderGraph* renderGraph, ImageViewProxyId imageViewProxyId)
			: renderGraph(renderGraph), imageViewProxyId(imageViewProxyId) {}

		void             Reset();
		ImageViewProxyId Id() const { return imageViewProxyId; }
		void             SetDebugName(std::string name) const;
	...
	struct BufferHandleInfo
	{
		...
		BufferHandleInfo(RenderGraph* renderGraph, BufferProxyId bufferProxyId)
			: renderGraph(renderGraph), bufferProxyId(bufferProxyId) {}

		void           Reset();
		BufferProxyId  Id() const { return bufferProxyId; }
	...
	// ── Pass context & descriptors (merged from RenderPassInfo.h) ───────────
	struct PassContext
	{
	...
	struct ComputePassDesc
	{
		ComputePassDesc()
		{
			...
			profilerTaskColor = Colors::belizeHole;
		}
	...
	struct TransferPassDesc
	{
		TransferPassDesc()
		{
			...
			profilerTaskColor = Colors::silver;
		}
		TransferPassDesc& SetSrcImages(std::vector<ImageViewProxyId>&& v) { srcImageViewProxies = std::move(v); return *this; }
		TransferPassDesc& SetDstImages(std::vector<ImageViewProxyId>&& v) { dstImageViewProxies = std::move(v); return *this; }
		TransferPassDesc& SetSrcBuffers(std::vector<BufferProxyId>&& v)   { srcBufferProxies    = std::move(v); return *this; }
		TransferPassDesc& SetDstBuffers(std::vector<BufferProxyId>&& v)   { dstBufferProxies    = std::move(v); return *this; }
		TransferPassDesc& SetRecordFunc(std::function<void(PassContext)> f){ recordFunc = f; return *this; }
		TransferPassDesc& SetProfilerInfo(uint32_t c, std::string n)      { profilerTaskColor = c; profilerTaskName = n; return *this; }
	...
	{
		ImagePresentPassDesc& SetImage(ImageViewProxyId id) { presentImageViewProxyId = id; return *this; }
		ImageViewProxyId presentImageViewProxyId;
	};

	struct FrameSyncBeginPassDesc {};
	struct FrameSyncEndPassDesc   {};
	// ── End Pass context & descriptors ───────────────────────────────────────
	...
	public:
		...
		using ImageProxyUnique = RAIIHandle<ImageHandleInfo, RenderGraph>;
		using ImageViewProxyUnique = RAIIHandle<ImageViewHandleInfo, RenderGraph>;
		using BufferProxyUnique = RAIIHandle<BufferHandleInfo, RenderGraph>;
		...
		ImageProxyUnique AddExternalImage(ImageData* image)
		{
			ImageProxy imageProxy;
		...
		ImageViewProxyUnique AddExternalImageView(ImageView* imageView, ImageUsageTypes usageType = ImageUsageTypes::Unknown)
		{
			ImageViewProxy imageViewProxy;
		...
		BufferProxyUnique AddExternalBuffer(Buffer* buffer)
		{
			BufferProxy bufferProxy;
		...
		ImageData *GetResolvedImage(size_t taskIndex, ImageProxyId imageProxy)
		{
	...
	private:
		...
		ProfilerTask CreateProfilerTask(const RenderPassDesc& renderPassDesc)
		{
			ProfilerTask task;
			task.startTime = -1.0f;
			task.endTime = -1.0f;
		...
		ProfilerTask CreateProfilerTask(const ComputePassDesc& computePassDesc)
		{
			ProfilerTask task;
			task.startTime = -1.0f;
			task.endTime = -1.0f;
		...
		ProfilerTask CreateProfilerTask(const TransferPassDesc& transferPassDesc)
		{
			ProfilerTask task;
			task.startTime = -1.0f;
			task.endTime = -1.0f;
		...
		ProfilerTask CreateProfilerTask(const ImagePresentPassDesc& imagePresentPassDesc)
		{
			ProfilerTask task;
			task.startTime = -1.0f;
			task.endTime = -1.0f;
		...
		ProfilerTask CreateProfilerTask(const FrameSyncBeginPassDesc& frameSyncBeginPassDesc)
		{
			ProfilerTask task;
			task.startTime = -1.0f;
			task.endTime = -1.0f;
		...
		ProfilerTask CreateProfilerTask(const FrameSyncEndPassDesc& frameSyncEndPassDesc)
		{
			ProfilerTask task;
			task.startTime = -1.0f;
			task.endTime = -1.0f;
	...
	// ── HandleInfo method bodies (need complete RenderGraph definition) ──────
	inline void ImageHandleInfo::Reset()                          { renderGraph->DeleteImage(imageProxyId); }
	inline void ImageHandleInfo::SetDebugName(std::string name) const { renderGraph->SetImageProxyDebugName(imageProxyId, name); }

	inline void ImageViewHandleInfo::Reset()                          { renderGraph->DeleteImageView(imageViewProxyId); }
