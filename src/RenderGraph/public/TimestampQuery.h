...
namespace cyRenderGraph
{
	struct TimestampQuery
	{
		...
		{
			commandBuffer.resetQueryPool(queryPool.get(), 0, static_cast<uint32_t>(timestampDatum.size()));
		...
		QueryResult QueryResults(vk::Device logicalDevice)
		{
			...
			if (queryRes == vk::Result::eSuccess)
			{
				...
				{
					timestampDatum[timestampIndex].time = (queryResults[timestampIndex] - queryResults[0]) * static_cast<double>(timestampPeriod / 1e9); //in seconds
				}
			}
