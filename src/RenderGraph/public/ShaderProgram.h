...
namespace cyRenderGraph
{
	...
	struct ImageSamplerBinding
	{
		...
		ImageSamplerBinding(ImageView* _imageView, Sampler* _sampler, uint32_t _shaderBindingId) : imageView(_imageView), sampler(_sampler), shaderBindingId(_shaderBindingId)
		{
			assert(_imageView);
			assert(_sampler);
		}
	...
	struct UniformBufferBinding
	{
		...
		UniformBufferBinding(Buffer* _buffer, uint32_t _shaderBindingId, vk::DeviceSize _offset, vk::DeviceSize _size) : buffer(_buffer), shaderBindingId(_shaderBindingId), offset(_offset), size(_size)
		{
			assert(_buffer);
		}
	...
	struct StorageBufferBinding
	{
		...
		StorageBufferBinding(Buffer* _buffer, uint32_t _shaderBindingId, vk::DeviceSize _offset, vk::DeviceSize _size) : buffer(_buffer), shaderBindingId(_shaderBindingId), offset(_offset), size(_size)
		{
			assert(_buffer);
		}
	...
	struct StorageImageBinding
	{
		...
		StorageImageBinding(ImageView* _imageView, uint32_t _shaderBindingId) : imageView(_imageView), shaderBindingId(_shaderBindingId)
		{
			assert(_imageView);
		}
	...
	public:
		...
		UniformBufferBinding MakeUniformBufferBinding(std::string bufferName, Buffer* _buffer, vk::DeviceSize _offset = 0, vk::DeviceSize _size = VK_WHOLE_SIZE) const
		{
		...
		StorageBufferBinding MakeStorageBufferBinding(std::string bufferName, Buffer* _buffer, vk::DeviceSize _offset = 0, vk::DeviceSize _size = VK_WHOLE_SIZE) const
		{
		...
		StorageBufferBinding MakeCheckedStorageBufferBinding(std::string bufferName, Buffer* _buffer, vk::DeviceSize _offset = 0, vk::DeviceSize _size = VK_WHOLE_SIZE) const
		{
			auto storageBufferId = GetStorageBufferId(bufferName);
			assert(storageBufferId.IsValid());
			auto storageBufferInfo = GetStorageBufferInfo(storageBufferId);
		...
		ImageSamplerBinding MakeImageSamplerBinding(std::string imageSamplerName, ImageView* _imageView, Sampler* _sampler) const
		{
		...
		StorageImageBinding MakeStorageImageBinding(std::string imageName, ImageView* _imageView) const
		{
				...
				for (size_t setIndex = 0; setIndex < setsCount; setIndex++)
				{
					...
					if (!dstUniformBufferId.IsValid())
					{
						...
						for (auto srcUniformId : srcUniformBuffer.uniformIds)
						{
							...
							UniformId dstUniformId = DescriptorSetLayoutKey::UniformId(res.uniformDatum.size());
							res.uniformDatum.push_back(UniformData());
							UniformData& dstUniformData = res.uniformDatum.back();
	...
	private:
		void RebuildIndex()
		{
			...
			{
				auto uniformId = UniformId(uniformIndex);
			...
			{
				auto uniformBufferId = UniformBufferId(uniformBufferIndex);
			...
			{
				auto imageSamplerId = ImageSamplerId(imageSamplerIndex);
			...
			{
				auto storageBufferId = StorageBufferId(storageBufferIndex);
			...
			{
				auto storageImageId = StorageImageId(storageImageIndex);
	...
	public:
			...
			: vertexShader(_vertexShader), fragmentShader(_fragmentShader)
		{
			...
			for (size_t setIndex = 0; setIndex < combinedDescriptorSetLayoutKeys.size(); setIndex++)
			{
				...
				std::vector<DescriptorSetLayoutKey> setLayoutStageKeys;
