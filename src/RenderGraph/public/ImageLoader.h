...
namespace cyRenderGraph
{
				...
				for (size_t z = 0; z < size.z; z++)
				{
					for (size_t y = 0; y < size.y; y++)
					{
						for (size_t x = 0; x < size.x; x++)
						{
							...
							texelPtr[0] = static_cast<uint8_t>(x);
							texelPtr[1] = static_cast<uint8_t>(y);
							texelPtr[2] = 0;
							texelPtr[3] = 0;
						}
					}
	...
	static gli::texture LoadTexelDataToGli(ImageTexelData texelData)
	{
		...
		for (size_t mipLevel = 0; mipLevel < texelData.mips.size(); mipLevel++)
		{
			...
			for (size_t layerIndex = 0; layerIndex < texelData.layersCount; layerIndex++)
			{
				...
				uint8_t* dstTexelsPtr = static_cast<uint8_t*>(texture.data(layerIndex, 0, mipLevel));
	...
	static void LoadTexelData(Core* core, const ImageTexelData* texelData, ImageData* dstImageData, ImageUsageTypes dstUsageType = ImageUsageTypes::GraphicsShaderRead)
	{
		...
		for (uint32_t mipLevel = 0; mipLevel < static_cast<uint32_t>(texelData->mips.size()); mipLevel++)
		{
			const auto& mip = texelData->mips[mipLevel];

			for (uint32_t arrayLayer = 0; arrayLayer < static_cast<uint32_t>(mip.layers.size()); arrayLayer++)
			{
