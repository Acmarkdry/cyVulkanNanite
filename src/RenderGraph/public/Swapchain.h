...
namespace cyRenderGraph
{
	...
	private:
		Swapchain(vk::Instance instance, vk::PhysicalDevice physicalDevice, vk::Device logicalDevice, WindowDesc windowDesc, glm::uvec2 defaultSize, uint32_t imagesCount, QueueFamilyIndices queueFamilyIndices, vk::PresentModeKHR preferredMode) : queue_family_indices_(std::move(queueFamilyIndices))
		{
			this->logicalDevice = logicalDevice;
			this->surface = cyRenderGraph::CreateSurface(instance, windowDesc);
			...
			auto swapchainCreateInfo = vk::SwapchainCreateInfoKHR()
			                           ...
			                           .setImageFormat(surfaceFormat.format) // 图片格式 如 eB8G8R8A8Srgb
			                           .setImageColorSpace(surfaceFormat.colorSpace) // 色彩空间
			                           .setImageExtent(extent) // 分辨率
			                           .setImageArrayLayers(1) // 1代表2d屏幕渲染，2用于vr的双眼立体渲染
			                           .setImageUsage(vk::ImageUsageFlagBits::eColorAttachment)
			                           .setPreTransform(surfaceDetails.capabilities.currentTransform) // 移动端专用适配
			...
			{
				// 因为要被多个queue访问，所以要设置为concurrent
				swapchainCreateInfo
