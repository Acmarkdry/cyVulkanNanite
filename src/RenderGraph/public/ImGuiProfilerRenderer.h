...
namespace ImGuiUtils
{
	...
	public:
		...
		void LoadFrameData(const cyRenderGraph::ProfilerTask* tasks, size_t count)
		{
	...
	private:
		...
		void RenderGraph(ImDrawList* drawList, glm::vec2 graphPos, glm::vec2 graphSize, size_t frameIndexOffset, float maxFrameTime)
		{
			...
			for (size_t frameNumber = 0; frameNumber < frames.size(); frameNumber++)
			{
				...
				{
					float taskStartHeight = (static_cast<float>(task.startTime) / maxFrameTime) * graphSize.y;
					float taskEndHeight = (static_cast<float>(task.endTime) / maxFrameTime) * graphSize.y;
					//taskMaxCosts[task.name] = std::max(taskMaxCosts[task.name], task.endTime - task.startTime);
					if (abs(taskEndHeight - taskStartHeight) > heightThreshold) Rect(drawList, taskPos + glm::vec2(0.0f, -taskStartHeight), taskPos + glm::vec2(frameWidth, -taskEndHeight), task.color, true);
				}
			}
		}
		...
		void RenderLegend(ImDrawList* drawList, glm::vec2 legendPos, glm::vec2 legendSize, size_t frameIndexOffset, float maxFrameTime)
		{
			...
			for (size_t taskIndex = 0; taskIndex < currFrame.tasks.size(); taskIndex++)
			{
				...
				uint32_t textColor = useColoredLegendText ? task.color : cyRenderGraph::Colors::imguiText; // task.color;
		...
		{
			if (filled) drawList->AddTriangleFilled(ImVec2(points[0].x, points[0].y), ImVec2(points[1].x, points[1].y), ImVec2(points[2].x, points[2].y), col);
			else drawList->AddTriangle(ImVec2(points[0].x, points[0].y), ImVec2(points[1].x, points[1].y), ImVec2(points[2].x, points[2].y), col);
		}
		...
		static void RenderTaskMarker(ImDrawList* drawList, glm::vec2 leftMinPoint, glm::vec2 leftMaxPoint, glm::vec2 rightMinPoint, glm::vec2 rightMaxPoint, uint32_t col)
		{
			...
			std::array<ImVec2, 4> points = {ImVec2(leftMaxPoint.x, leftMinPoint.y), ImVec2(leftMaxPoint.x, leftMaxPoint.y), ImVec2(rightMinPoint.x, rightMaxPoint.y), ImVec2(rightMinPoint.x, rightMinPoint.y)};
		...
		struct FrameData
		{
			...
			std::vector<cyRenderGraph::ProfilerTask> tasks;
	...
	{
	public:
		ProfilersWindow(float maxFrameTime = 1.0f / 60.0f): cpuGraph(300), gpuGraph(300), maxFrameTime(maxFrameTime)
		{
			stopProfiling = false;
			frameOffset = 0;
			frameWidth = 3;
		...
		void Render()
		{
			...
			int sizeMargin = static_cast<int>(ImGui::GetStyle().ItemSpacing.y);
			int maxGraphHeight = 300;
			int availableGraphHeight = (static_cast<int>(canvasSize.y) - sizeMargin) / 2;
			int graphHeight = std::min(maxGraphHeight, availableGraphHeight);
			int legendWidth = 200;
			int graphWidth = static_cast<int>(canvasSize.x) - legendWidth;
