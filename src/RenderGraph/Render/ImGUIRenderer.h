...
{
public:
	ImGuiRenderer(cyRenderGraph::Core* _core, GLFWwindow* window)
	{
		this->core = _core;
		...
		imageSpaceSampler.reset(new cyRenderGraph::Sampler(core->GetLogicalDevice(), vk::SamplerAddressMode::eClampToEdge, vk::Filter::eLinear, vk::SamplerMipmapMode::eNearest));
		ReloadShaders();
	...
	void RenderFrame(const cyRenderGraph::InFlightQueue::FrameInfo& frameInfo, GLFWwindow* window, ImDrawData* drawData)
	{
		...
		core->GetRenderGraph()->AddPass(cyRenderGraph::RenderPassDesc().SetColorAttachments({frameInfo.swapchainImageViewProxyId}, vk::AttachmentLoadOp::eLoad).SetRenderAreaExtent(viewportExtent).SetProfilerInfo(cyRenderGraph::Colors::peterRiver, "ImGuiPass").SetRecordFunc([this, frameInfo, drawData, vertexBuffer, indexBuffer](cyRenderGraph::RenderPassContext passContext)
		{
			auto pipeineInfo = this->core->GetPipelineCache()->BindGraphicsPipeline(passContext.GetCommandBuffer(), passContext.GetRenderPass()->GetHandle(), cyRenderGraph::DepthSettings::Disabled(), {cyRenderGraph::BlendSettings::AlphaBlend()}, GetImGuiVertexDeclaration(), vk::PrimitiveTopology::eTriangleList, imGuiShader.program.get());
			{
				...
				{
				  const cyRenderGraph::DescriptorSetLayoutKey *vertexDataSetInfo = debugRendererShader.vertex->GetSetInfo(VertexDataSetIndex);
				  ...
				  const cyRenderGraph::DescriptorSetLayoutKey *fragmentDataSetInfo = debugRendererShader.fragment->GetSetInfo(FragmentDataSetIndex);
				  std::vector<cyRenderGraph::ImageSamplerBinding> imageSamplerBindings;
				...
				const cyRenderGraph::DescriptorSetLayoutKey* shaderDataSetInfo = imGuiShader.program->GetSetInfo(ShaderDataSetIndex);
				auto shaderData = frameInfo.memoryPool->BeginSet(shaderDataSetInfo);
				{
					...
					shaderDataBuffer->projMatrix = glm::ortho(0.0f, static_cast<float>(viewportExtent.width), 0.0f, static_cast<float>(viewportExtent.height));
				}
				...
				const cyRenderGraph::DescriptorSetLayoutKey* drawCallSetInfo = imGuiShader.program->GetSetInfo(DrawCallDataSetIndex);
				...
				for (int cmdListIndex = 0; cmdListIndex < drawData->CmdListsCount; cmdListIndex++)
				{
					...
					for (int cmdIndex = 0; cmdIndex < cmdList->CmdBuffer.Size; cmdIndex++)
					{
						...
						else
						{
							...
							auto texImageView = static_cast<cyRenderGraph::ImageView*>(drawCmd->TextureId);
							cyRenderGraph::ImageSamplerBinding texBinding = drawCallSetInfo->MakeImageSamplerBinding("tex", texImageView, imageSpaceSampler.get());

							auto drawCallSet = this->core->GetDescriptorSetCache()->GetDescriptorSet(*drawCallSetInfo, {}, {}, {texBinding});

							passContext.GetCommandBuffer().bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipeineInfo.pipelineLayout, ShaderDataSetIndex, {shaderDataSet, drawCallSet}, {shaderData.dynamicOffset});
	...
	void ProcessInput(GLFWwindow* window)
	{
		...
		imguiIO.MousePos = ImVec2(static_cast<float>(mousePos.x), static_cast<float>(mousePos.y)); // set the mouse position
		...
		imguiIO.DeltaTime = inputState.lastUpdateTime > 0.0 ? static_cast<float>(currTime - inputState.lastUpdateTime) : 1.0f / 60.0f;
	...
	void ReloadShaders()
	{
		imGuiShader.vertex.reset(new cyRenderGraph::Shader(core->GetLogicalDevice(), "../data/Shaders/spirv/ImGui/ImGui.vert.spv"));
		imGuiShader.fragment.reset(new cyRenderGraph::Shader(core->GetLogicalDevice(), "../data/Shaders/spirv/ImGui/ImGui.frag.spv"));
		imGuiShader.program.reset(new cyRenderGraph::ShaderProgram(imGuiShader.vertex.get(), imGuiShader.fragment.get()));
	}
...
private:
	...
	{
		auto inputState = static_cast<InputState*>(glfwGetWindowUserPointer(window));
	...
	void UploadBuffers(cyRenderGraph::Buffer* vertexBuffer, cyRenderGraph::Buffer* indexBuffer, ImDrawData* drawData)
	{
		auto vertBufMemory = static_cast<ImDrawVert*>(vertexBuffer->Map());
		ImDrawIdx* indexBufMemory = static_cast<unsigned int*>(indexBuffer->Map());
...
#pragma pack(pop)
	...
	{
		FrameResources(cyRenderGraph::Core* core, size_t maxVerticesCount, size_t maxIndicesCount)
		{
			imGuiIndexBuffer = std::make_unique<cyRenderGraph::Buffer>(core->GetPhysicalDevice(), core->GetLogicalDevice(), sizeof(glm::uint32_t) * maxIndicesCount, vk::BufferUsageFlagBits::eIndexBuffer, vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);
			imGuiVertexBuffer = std::make_unique<cyRenderGraph::Buffer>(core->GetPhysicalDevice(), core->GetLogicalDevice(), sizeof(ImGuiVertex) * maxVerticesCount, vk::BufferUsageFlagBits::eVertexBuffer, vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);
		}

		std::unique_ptr<cyRenderGraph::Buffer> imGuiIndexBuffer;
		std::unique_ptr<cyRenderGraph::Buffer> imGuiVertexBuffer;
	};
	...
	void LoadImguiFont()
	{
		...
		auto texelData = cyRenderGraph::CreateSimpleImageTexelData(pixels, width, height);
		auto fontCreateDesc = cyRenderGraph::Image::CreateInfo2d(glm::uvec2(texelData.baseSize.x, texelData.baseSize.y), static_cast<uint32_t>(texelData.mips.size()), 1, texelData.format, vk::ImageUsageFlagBits::eSampled | vk::ImageUsageFlagBits::eTransferDst);

		this->fontImage = std::make_unique<cyRenderGraph::Image>(core->GetPhysicalDevice(), core->GetLogicalDevice(), fontCreateDesc);
		cyRenderGraph::LoadTexelData(core, &texelData, fontImage->GetImageData());
		this->fontImageView = std::make_unique<cyRenderGraph::ImageView>(core->GetLogicalDevice(), fontImage->GetImageData(), 0, fontImage->GetImageData()->GetMipsCount(), 0, 1);

		imGuiIO.Fonts->TexID = static_cast<void*>(fontImageView.get());
	}

	static constexpr uint32_t ShaderDataSetIndex = 0;
	static constexpr uint32_t DrawCallDataSetIndex = 1;

	static cyRenderGraph::VertexDeclaration GetImGuiVertexDeclaration()
	{
		cyRenderGraph::VertexDeclaration vertexDecl;
		vertexDecl.AddVertexInputBinding(0, sizeof(ImDrawVert));
		vertexDecl.AddVertexAttribute(0, offsetof(ImDrawVert, pos), cyRenderGraph::AttribTypes::vec2, 0);
		vertexDecl.AddVertexAttribute(0, offsetof(ImDrawVert, uv), cyRenderGraph::AttribTypes::vec2, 1);
		vertexDecl.AddVertexAttribute(0, offsetof(ImDrawVert, col), cyRenderGraph::AttribTypes::color32, 2);
...
#pragma pack(pop)
		...
		std::unique_ptr<cyRenderGraph::Shader> vertex;
		std::unique_ptr<cyRenderGraph::Shader> fragment;
		std::unique_ptr<cyRenderGraph::ShaderProgram> program;
	} imGuiShader;

	vk::Extent2D viewportExtent;

	std::unique_ptr<cyRenderGraph::ImageView> fontImageView;
	std::unique_ptr<cyRenderGraph::Image> fontImage;

	std::unique_ptr<cyRenderGraph::Sampler> imageSpaceSampler;
