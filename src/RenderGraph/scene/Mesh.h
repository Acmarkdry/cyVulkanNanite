...
// TODO: tinyobj 加载暂未启用，MeshData 可通过外部数据直接构造
...
struct MeshData
{
	...
	*/
	...
	static MeshData GeneratePointMesh(MeshData srcMesh, float density)
	{
		...
		{
			glm::vec3 points[3];
			for (size_t vertexNumber = 0; vertexNumber < 3; vertexNumber++) points[vertexNumber] = srcMesh.vertices[srcMesh.indices[triangleIndex * 3 + vertexNumber]].pos;
		...
		for (size_t pointIndex = 0; pointIndex < pointsCount; pointIndex++)
		{
			...
			if (it == triangleAreas.end()) continue;

			size_t triangleIndex = it - triangleAreas.begin();

			Vertex triangleVertices[3];
			for (size_t vertexNumber = 0; vertexNumber < 3; vertexNumber++) triangleVertices[vertexNumber] = srcMesh.vertices[srcMesh.indices[triangleIndex * 3 + vertexNumber]];
...
struct Mesh
{
	Mesh(const MeshData& meshData, vk::PhysicalDevice physicalDevice, vk::Device logicalDevice, vk::CommandBuffer transferCommandBuffer)
	{
		...
		vertexBuffer = std::make_unique<cyRenderGraph::StagedBuffer>(physicalDevice, logicalDevice, meshData.vertices.size() * sizeof(MeshData::Vertex), vk::BufferUsageFlagBits::eVertexBuffer);
		if (indicesCount > 0) indexBuffer = std::make_unique<cyRenderGraph::StagedBuffer>(physicalDevice, logicalDevice, meshData.indices.size() * sizeof(MeshData::IndexType), vk::BufferUsageFlagBits::eIndexBuffer);
	...
	static cyRenderGraph::VertexDeclaration GetVertexDeclaration()
	{
		cyRenderGraph::VertexDeclaration vertexDecl;
		vertexDecl.AddVertexInputBinding(0, sizeof(MeshData::Vertex));
		vertexDecl.AddVertexAttribute(0, offsetof(MeshData::Vertex, pos), cyRenderGraph::AttribTypes::vec3, 0);
		vertexDecl.AddVertexAttribute(0, offsetof(MeshData::Vertex, normal), cyRenderGraph::AttribTypes::vec3, 1);
		vertexDecl.AddVertexAttribute(0, offsetof(MeshData::Vertex, uv), cyRenderGraph::AttribTypes::vec2, 2);
